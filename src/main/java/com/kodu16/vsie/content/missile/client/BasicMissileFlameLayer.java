...
public class BasicMissileFlameLayer extends GeoRenderLayer<AbstractMissileEntity> {
    private static final String NOZZLE_BONE_NAME = "nozzle";
    private static final float FLAME_LENGTH = 24.0F;
    private static final float BASE_RADIUS = 0.16F;
    private static final float TIP_RADIUS = 0.03F;
    ...
    private static final float M_2PI = (float) (Math.PI * 2.0D);

    public BasicMissileFlameLayer(GeoRenderer<AbstractMissileEntity> entityRendererIn) {
        super(entityRendererIn);
    }

    @Override
    public void render(PoseStack poseStack, AbstractMissileEntity animatable, BakedGeoModel bakedModel,
                       RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer,
                       float partialTick, int packedLight, int packedOverlay) {
        // Function: basic missiles keep a fixed engine plume while the entity renderer is active.
        super.render(poseStack, animatable, bakedModel, renderType, bufferSource, buffer, partialTick, packedLight, packedOverlay);
    }

    @Override
    public void renderForBone(PoseStack poseStack, AbstractMissileEntity animatable, GeoBone bone,
                              RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer,
                              float partialTick, int packedLight, int packedOverlay) {
        ...
        long seed = animatable.getId() ^ 0xA54FF53A5F1D36F1L;
        float time = animatable.tickCount + partialTick;

        for (int i = 0; i <= LENGTH_SEGMENTS; i++) {
            float t = i / (float) LENGTH_SEGMENTS;
            float y = -t * FLAME_LENGTH;
            float radius = lerp(BASE_RADIUS, TIP_RADIUS, t);
            float r = lerp(0.6F, 0.6F, t);
            float g = lerp(0.7F, 0.4F, t);
            float b = lerp(0.3F, 0.9F, t);
            float a = lerp(0.85F, 0.0F, t);
            // Function: reuse the thruster flame jitter so missile exhaust keeps the same animated color language.
            float[] color = FlameColorJitter.apply(seed, time, t, r, g, b, a);
            // Function: the missile nozzle sits on the local rear axis, so the plume must extend backward along local -Y instead of sideways.
