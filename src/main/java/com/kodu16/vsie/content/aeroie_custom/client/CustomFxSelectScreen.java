...
/** Simple folder-rooted picker for Photon .fx files under AeroIE/fx. */
final class CustomFxSelectScreen extends CrispScreen {
    ...
    CustomFxSelectScreen(Consumer<String> onSelect, Runnable onCancel) {
        super(Component.translatable("screen.vsie.custom_turret_editor.fx_select"));
    ...
    @Override
    protected void init() {
        files = CustomFxResources.scanFxFiles();
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        int width = Math.min(420, this.width - 30);
        ...
        graphics.drawString(font, Component.translatable("screen.vsie.custom_turret_editor.fx_select"),
                left + 8, top + 8, ACCENT, false);
        graphics.drawString(font, Component.literal(CustomFxResources.root().toString()),
                left + 8, top + height - 18, MUTED, false);
        if (files.isEmpty()) {
            graphics.drawWordWrap(font, Component.translatable("screen.vsie.custom_turret_editor.fx_empty"),
        ...
        drawButton(graphics, left + width - 168, top + height - 44, 76, 20,
                Component.translatable("screen.vsie.custom_turret_editor.fx_clear"), false);
        drawButton(graphics, left + width - 86, top + height - 44, 76, 20,
                Component.translatable("screen.vsie.custom_turret_editor.fx_bind"), !files.isEmpty());
